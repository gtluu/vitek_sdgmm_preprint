...
edit_metadata <- function(dataset, spot_file, roi_list) {
  spots <- as.data.frame(read.table(spot_file, sep=" ", skip=2, col.names=c("x", "y", "spot", "region")))
...
preprocess_data <- function(dataset) {
  ...
  peaks <- peakPick(dataset, method='simple', SNR=6)
  peaks <- peakAlign(peaks, tolerance=0.2, units='mz')
  peaks <- peakFilter(peaks, freq.min=0.25)
  peaks <- process(peaks)
...
ims1 <- as(readMSIData('mini_ims.imzML'), 'MSContinuousImagingExperiment')
ims2 <- as(readMSIData('mini_ims_2.imzML'), 'MSContinuousImagingExperiment')

spots1 <- 'mini_ims.txt'
spots2 <- 'mini_ims_2.txt'
...
save.image('D:/code/vitek_sdgmm_preprint/k12/sdgmm_test_v3_mini.Rdata')
...
fuzzy_peaks <- generate_ref_peak_list(list(data1, data2))
#ref_peaks <- unique(fuzzy_join$mean)

ims_data_list <- peak_process_reference(list(data1, data2), fuzzy_peaks)
...
register(SnowParam(workers=(detectCores()-1), progressbar=TRUE, log=TRUE, stop.on.error=FALSE, logdir=getwd()))
set.seed(233)
...
summary(segtest)
topFeatures(segtest, n=length(segtest))
