setwd('C:/Users/gordon/Downloads/sdgmm')
...
check_bad_features <- function(dataset, r=1, k=2, compare=TRUE) {
  ...
  print(bad_features_vector)
  if (length(bad_features_vector) != 0) {
    bad_mz_vector <- mz(dataset)
    for (bad_feature in sort(bad_features_vector, decreasing=TRUE)) {
      dataset <- dataset[features(dataset, mz < bad_mz_vector[bad_feature] - 0.0001 | mz > bad_mz_vector[bad_feature] + 0.0001, envir=environment()),]
      print(dataset)
    }
...
save.image("C:/Users/gordon/Downloads/sdgmm/sdgmm_test.RData")

#ref_peaks <- generate_ref_peak_list(list(data1, data2))
...
#means_test <- meansTest(combined, ~ region, groups=run(combined))
#summary(means_test)

#save.image("C:/Users/gordon/Downloads/sdgmm/sdgmm_test.RData")

register(SnowParam(workers=(detectCores()-2), progressbar=TRUE, log=TRUE, stop.on.error=FALSE, logdir=getwd()))
#register(SerialParam(stop.on.error=FALSE, log=TRUE, logdir=getwd()))
...
sdgmm1 <- spatialDGMM(ims_data_list[[1]][['processed_reference']], r=1, k=2, groups=run(ims_data_list[[1]][['processed_reference']]))
sdgmm2 <- spatialDGMM(ims_data_list[[2]][['processed_reference']], r=1, k=2, groups=run(ims_data_list[[2]][['processed_reference']]))

save.image("C:/Users/gordon/Downloads/sdgmm/sdgmm_test.RData")

sdgmm <- Cardinal::combine(sdgmm1, sdgmm2)
...
if (length(bad_features_vector) != 0) {
  ...
  register(SnowParam(workers=(detectCores()-2), progressbar=TRUE))
  if (compare == FALSE) {
    #return(spatialDGMM(combined, r=r, k=k, init="kmeans", iter.max=100, tol=1e11))
  } else {
    #return(spatialDGMM(combined, r=r, k=k, init="kmeans", iter.max=100, tol=1e11, groups=run(combined)))
  }
}
